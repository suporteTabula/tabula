...
<div class="panel panel-default">
	...
	<div class="panel-body">
		<form action="{{ route('course.chapter.update.teacher', ['id' => $chapter->id]) }}" method="post" enctype="multipart/form-data">
			{{ csrf_field() }}
			...
			<div class="form-group">
				<div class="text-center">
					...
					<a class="btn btn-success" href="{{ route('course.edit.teacher', ['id' => $chapter->course_id]) }}">Voltar</a>
				</div>
			</div>
		</form>
		...
		<div class="title_chapter">
			...
			<button id="create-complement">Novo Material Complementar</button>
		</div>

		<div id="lesson">
			<form action="{{ route('course.item.teacher', ['id' => $chapter->id]) }}" method="post" enctype="multipart/form-data">
				{{ csrf_field() }}
		...
		<div id="complement">
			<form action="{{ route('course.item.teacher', ['id' => $chapter->id]) }}" method="post" enctype="multipart/form-data">
				...
				<div id="arquivo">
					<div class="form-group">
						<label for="desc">Adicionar Arquivo ('.jpg', '.pdf', '.zip')</label>
						<input class="form-control" type="file" name="archive">
					</div>
				</div>
				...
				<div class="form-group">
					<div class="text-center">
						<button class="btn btn-success" type="submit">Novo Material Complementar</button>
						{{--<a class="btn btn-success" href="{{ route('courses.teacher') }}">Voltar</a>--}}
					</div>
				</div>
			</form>
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<a href="{{ route('course.item.edit.teacher', ['id' => $item->id]) }}">
							<img style=" width:35px; " src="{{asset('images\edit.svg')}}">
						</a>
					</td>
					<td>
						<a class="remove-record" data-toggle="modal" data-id={{$item->id}} data-target="#custom-width-modal" data-url="{{ route('course.item.delete.teacher', ['id' => $item->id]) }} ">
		...
		@if ($items->count() > 0)
		<form action="{{ route('course.item.delete.teacher', ['id' => $item->id]) }}" method="GET" class="remove-record-model">
			{{ csrf_field() }}
...
<script>
	$( function() {
		var dialogLesson, dialogTest, dialogComplement, form,
		name = $( "#name" ),
		desc = $( "#desc" ),
...
//Complemento
dialogComplement = $( "#complement" ).dialog({
	autoOpen: false,
	height: 450,
	width: 350,
	modal: true,
...
form = dialogComplement.find( "form" ).on( "submit", function( event ) {				    	
});

$( "#create-complement" ).button().on( "click", function() {
	dialogComplement.dialog( "open" );
});
});
</script>
